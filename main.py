...
quotes = ["We cannot solve problems with the kind of thinking we employed when we came up with them.", "Learn as if you will live forever, live like you will die tomorrow.","Stay away from those people who try to disparage your ambitions. Small minds will always do that, but great minds will give you a feeling that you can become great too.","When you give joy to other people, you get more joy in return. You should give a good thought to happiness that you can give out.","When you change your thoughts, remember to also change your world.","It is only when we take chances, when our lives improve. The initial and the most difficult risk that we need to take is to become honest.","Nature has given us all the pieces required to achieve exceptional wellness and health, but has left it to us to put these pieces together.","Who do you think you're talking to? Who is is that you think you see? Do you know how much I make a year? Well even if I told you, you wouldn't get it. You know what would happen if i suddenly decided to stop going into work one day? A company, big enough to listed on the NASDAQ, goes belly up, disappears. It ceases to exist without me. You clearly don't know who you're talking to, so let me clue you in. I am not in danger Skylar, I am the danger. A guy opens his door and gets shot, you think that of me. No. **I am the one who knocks**"]
...
images = ['https://imgur.com/gallery/c1D3W','https://imgur.com/gallery/WnfH3','https://imgur.com/gallery/jluCLzA','https://imgur.com/gallery/gFhGNWP','https://imgur.com/gallery/eKZTF','https://i.imgur.com/EqQsfEa.jpeg','https://i.imgur.com/4yRuKQW.jpeg','https://i.imgur.com/1d1vstm.jpeg','https://i.imgur.com/P6ZWxFw.jpeg','https://i.imgur.com/LHJeP2y.jpeg','https://i.imgur.com/bPJuv8u.jpeg','https://i.imgur.com/rhupINi.jpeg','https://i.imgur.com/OxcLDLu.jpeg','https://i.imgur.com/vUUOrlZ.jpeg','https://i.imgur.com/p8jqPD8.jpeg','https://i.imgur.com/8ov6rgV.jpeg','https://i.imgur.com/pR98zM2.jpeg','https://i.imgur.com/GwMATgy.jpeg','https://i.imgur.com/ZkXErCc.jpeg']
# 19, 0-18
...
async def on_message(message):
  ...
  if message.content == '%carquiz':
    ...
    print(qSelected)
    print(aSelected)
    msg1_loc = message.channel
    
    
    action = await bot.wait_for('message')
    print(action.content)
    user = action.author.name
    msg2_loc = message.channel
    
    
    if action.content == aSelected and msg1_loc == msg2_loc and action.author.name == user:
      await ctx.send(f"Correct, good job {user}")
    elif action.content != aSelected and msg1_loc == msg2_loc and action.author.name == user:
  ...
  if message.content == '%help':
    ctx = message.channel
    await ctx.send("List of commands: (%hello: bot replies with sup), (%quote: bot gives you a random quote), (%8ball {question}: will give you a randomized answer), (%simonsays {text}: bot will repeat {text} back to you), (%meme: generates a random breaking bad meme), (%gif: generates a random breaking bad gif), (%carquiz: quizzes your knowledge of car brands)")

  if message.content == '%sexyunderagegirl':
    ctx = message.channel
    await ctx.send('https://m.media-amazon.com/images/M/MV5BNmU1MmIzMzgtYTViNS00YTgxLTkyMDAtZTljNWUwYmUyOTc5XkEyXkFqcGdeQXVyMTI3MzE3NjA5._V1_.jpg')
    await ctx.send('You have been added to the pedo list.')
